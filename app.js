/**
 * Imazix - Minimalist Quotation & Invoice Studio
...
(function () {
  ...
  const state = {
    ...
    client: {
      name: 'Divinely Bangladesh',
      details: 'Attn: Project Coordinator\nDhaka, Bangladesh\nEmail: hello@divinelybangladesh.com'
    },
    
    items: [
      { id: 1, description: 'Video Editing Service', qty: 1, price: 50000, discountRate: 10 },
      { id: 2, description: 'Website Design & Development', qty: 1, price: 80000, discountRate: 5 }
    ],
  ...
  const els = {
    ...
    docClientLabel: document.getElementById('docClientLabel'),
    ...
    docItemDiscountRow: document.getElementById('docItemDiscountRow'),
    docItemDiscountAmtView: document.getElementById('docItemDiscountAmtView'),
  ...
  function normalizeItems(items) {
    ...
    return items.map((item, index) => ({
      ...
      discountRate: clampPercent(item.discountRate)
    }));
  }
  ...
  function setMode(mode) {
    const previousMode = state.type;
    state.type = mode;
    if (previousMode !== mode) {
      if (mode === 'invoice') {
        state.docNumber = state.docNumber.replace(/^QT-/i, 'INV-');
        if (!state.docNumber.startsWith('INV-')) state.docNumber = `INV-${state.docNumber}`;
        state.status = 'PENDING';
        state.dueDate = getFutureDate(14);
        if (state.notes === DEFAULT_QUOTATION_NOTES) state.notes = DEFAULT_INVOICE_NOTES;
      } else {
        state.docNumber = state.docNumber.replace(/^INV-/i, 'QT-');
        if (!state.docNumber.startsWith('QT-')) state.docNumber = `QT-${state.docNumber}`;
        state.status = 'SENT';
        state.dueDate = getFutureDate(14);
        if (state.notes === DEFAULT_INVOICE_NOTES) state.notes = DEFAULT_QUOTATION_NOTES;
      }
  ...
  function convertDocumentType() {
    setMode(state.type === 'quotation' ? 'invoice' : 'quotation');
  }
  ...
  function renderItemsEditor() {
      ...
      tr.innerHTML = `
        ...
        <td>
          <div class="item-discount-wrap">
            <input type="number" class="form-control item-discount" data-idx="${idx}" value="${clampPercent(item.discountRate)}" min="0" max="100" step="0.5" aria-label="Item discount percentage">
          </div>
        </td>
    ...
    const discountInputs = els.itemsEditorBody.querySelectorAll('.item-discount');
    ...
    discountInputs.forEach(input => {
      input.addEventListener('input', (e) => {
        const idx = e.target.getAttribute('data-idx');
        state.items[idx].discountRate = clampPercent(e.target.value);
        render();
      });
      input.addEventListener('change', (e) => {
        e.target.value = clampPercent(e.target.value);
      });
    });
    ...
    removeBtns.forEach(btn => {
      btn.addEventListener('click', (e) => {
        ...
        if (state.items.length <= 1) {
          state.items[0] = { id: Date.now(), description: '', qty: 1, price: 0, discountRate: 0 };
        } else {
  ...
  function render() {
    ...
    els.docClientLabel.textContent = isQuotation ? 'Prepared For' : 'Bill To';
    els.docNotesTitle.textContent = isQuotation ? 'Quotation Terms' : 'Invoice Notes';
    ...
    // Sender & Client Views
    els.docSenderName.textContent = state.sender.name || 'Imazix';
    ...
    els.docPreparedByName.textContent = state.sender.name || 'Imazix';
    els.docPreparedByDetails.textContent = 'Creative & Digital Team';
    els.docSignatureLabel.textContent = `Authorized Signature (${state.sender.name || 'Imazix'})`;
    ...
    state.items.forEach((item, index) => {
      const discountRate = clampPercent(item.discountRate);
      const originalUnitPrice = Number(item.price) || 0;
      const discountedUnitPrice = originalUnitPrice * (1 - discountRate / 100);
      const originalItemTotal = (Number(item.qty) || 0) * originalUnitPrice;
      const itemDiscount = originalItemTotal * discountRate / 100;
      const itemTotal = originalItemTotal - itemDiscount;
      subtotal += originalItemTotal;
      itemDiscountTotal += itemDiscount;

      const unitPriceHtml = discountRate > 0
        ? `<div class="price-stack"><span class="price-original">${formatMoney(originalUnitPrice, state.currency)}</span><span class="price-discounted">${formatMoney(discountedUnitPrice, state.currency)}</span></div>`
        : formatMoney(originalUnitPrice, state.currency);
      ...
      tr.innerHTML = `
        ...
        <td class="right">${unitPriceHtml}</td>
        <td class="center">${discountHtml}</td>
    ...
    const discountAmt = (afterItemDiscounts * clampPercent(state.discountRate)) / 100;
    const afterDiscount = afterItemDiscounts - discountAmt;
    ...
    if (itemDiscountTotal > 0) {
      els.docItemDiscountRow.style.display = 'flex';
      els.docItemDiscountAmtView.textContent = `-${formatMoney(itemDiscountTotal, state.currency)}`;
    } else {
      els.docItemDiscountRow.style.display = 'none';
    }
  ...
  function loadDemoData() {
    ...
    state.items = [
      { id: 1, description: 'Video Editing & Post-Production', qty: 2, price: 30000, discountRate: 10 },
      { id: 2, description: 'Website Design & Development', qty: 1, price: 80000, discountRate: 5 }
    ];
  ...
  function saveSenderDefaults() {
    try {
      ...
      alert('Imazix profile saved as default!');
    } catch (e) {
